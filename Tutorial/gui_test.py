...
class MyApp(QMainWindow):
	def __init__(self):
		...
		self.title_screen = self.findChild(QLabel, "label_widget")
		...
		self.actions = [self.action1, self.action2, self.action3, self.action4, self.action11]
	...
	def delete_screen(self):
		...
		# hay que rebisar si hay una seleccionada
		# hay que borrar en todas las listas el indice
		# hay que crear unn metodopara seleccionar uno

		# sender_action = self.sender()

        # # Obtener el índice del QAction en la lista de QAction
		# index = self.actions().index(sender_action)

        # # Verificar si el índice está dentro del rango válido
		# if 0 <= index < len(self.labels):
		# 	# Obtener la QLabel correspondiente al índice
		# 	label = self.labels[index]

		# 	# Remover la QLabel del layout
		# 	label.setParent(None)

        #     # Remover la QLabel de la lista
	...
	def firts_screen(self):
		...
		self.label_widget.setText(self.widgets[self.scree_prev])
	...
	# Esta función se llama cuando se hace clic en cualquier QLabel
		...
		self.label_widget.setText(self.widgets[self.labels.index(label)])
